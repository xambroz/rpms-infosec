...
sed -i -e 's|/usr/libexec/john|/usr/libexec/%{name}|' \
       -e 's|/usr/share/john|/usr/share/%{name}|' \
...
install -m 755 run/stats %{buildroot}%{_libexecdir}/%{name}/
install -d -m 755 %{buildroot}%{_datarootdir}/%{name}/extra/

# Copy link files
for LINK in $(find run/ -type l) ; do
    install "$LINK" %{buildroot}%{_bindir}/
done

# Configuration
install -m 755 run/*.conf %{buildroot}%{_sysconfdir}/%{name}

# Shared files
install -m 755 run/*.chr %{buildroot}%{_datarootdir}/%{name}/
