...
VCS:            https://svn.code.sf.net/p/guymager/code/tags/guymager-%{vesion}

License:        GPL

Group:          Applications/Forensics Tools

# Source0:      https://netix.dl.sourceforge.net/project/guymager/guymager/LatestSource/guymager-%%{version}.tar.gz
Source0:        https://downloads.sourceforge.net/guymager/guymager-%{version}.tar.gz
Patch1:         %{name}-%{version}-patch-001

BuildRequires:  gcc-c++
BuildRequires:  make
BuildRequires:  qt5-qtbase-devel
BuildRequires:  libguytools-devel
...
install -p -m 644 %{name}.cfg %{buildroot}%{_sysconfdir}/%{name}
sed --in-place 's/\\$/ /' %{buildroot}%{_sysconfdir}/%{name}/%{name}.cfg
install -p -m 644 manuals/%{name}.1 %{buildroot}%{_mandir}/man1
